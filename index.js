...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`root@IAmAdyy:~/Selfbot# ${err}`)
if (stdout) {
reply(stdout)
}
})
...
case 'japanesemenu':
conn.sendMessage(from, jpmenu(f), text, {quoted: ftroli})
break

case 'russianmenu':
conn.sendMessage(from, rumenu(f), text, {quoted: ftroli})
break
...
break
    ...
    case 'jadibot':
    case 'scan':
    if (mek.key.fromMe) return reply('tidak bisa jadibot kecuali member yang ingin jadibot')
...
buttonsMessage = {footerText:'© Adii', imageMessage: imageMsg,
...
.then(async (a) => {
 ...
 const captions = `*[ P L A Y  M U S I C ]*\n\n*T I T L S* : ${title}\n*Ext* : MP3\n*Size* : ${filesizeF}\n*Link* : ${a.data}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
...
reply(`Kirim gambar dengan caption ${prefix}setimagereply`)
}
break	
...
case 'sharelock':
if (args.length < 1) return reply(`Contoh : sharelock Adii|self`)
lan = `${args.join(' ')}`
send = lan.split("|")[0];
lok = lan.split("|")[1];
...
case 'src':
case 'sourcecode':
imgreply('Source code bot:\nhttps://github.com/IAmAdyy/Selfbot')
break
...
case 's':
anu = args.join(' ').split('|')
satu = anu[0] !== '' ? anu[0] : `IAmAdyy`
dua = typeof anu[1] !== 'undefined' ? anu[1] : `Selfbot`
if (isMedia && !mek.message.videoMessage || isQuotedImage ) {
const encmedia = isQuotedImage   ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
media = await conn.downloadAndSaveMediaMessage(encmedia)
